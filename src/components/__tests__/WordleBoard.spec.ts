...
describe('WordleBoard', () => {
  ...
  describe('End of the game messages', () => {
    ...
    describe.each(
      ...
      ({ numberOfGuesses, shouldSeeDefeatMessage }) => {
        test(`therefore for ${numberOfGuesses} guess(es), a defeat should ${shouldSeeDefeatMessage ? '' : 'not'} appear`, async () => {
  ...
  describe('Rules for defining the word of the day', () => {
    ...
    test.each([
      ...
      'since $reason: $wordOfTheDay is invalid, therefor a warning must be emitted',
      async ({ wordOfTheDay }) => {
        mount(WordleBoard, {
          props: {
            wordOfTheDay
  ...
  describe('Player input', () => {
    ...
    test('player loses control after the max amount of guesses have been sent', async () => {
      const guesses = ['WRONG', 'GUESS', 'HELLO', 'HAPPY', 'WORLD', 'CODER']
  ...
  test('All previous guesses done by the player are visible in the page', async () => {
    const guesses = ['WRONG', 'GUESS', 'HELLO', 'WORLD', 'CODER']
  ...
  describe('Displaying hints/feedback to the player', () => {
    test('feedback is not rendered before the player started typing their guess', async () => {
      expect(wrapper.find('[data-letter-feedback]').exists()).toBe(false)
    })

    test('feedback is not rendered while the player is typing their guess', async () => {
      await playerTypesGuess(wordOfTheDay)
      expect(wrapper.find('[data-letter-feedback]').exists()).toBe(false)
    })

    test('feedback is rendered after the player submitted their guess', async () => {
      await playerTypesAndSubmitsGuess(wordOfTheDay)
